...
sequenceDiagram
    ...
    Note over Router: Server sends initial messages
    ...
    Note over Router: Enter event loop - exactly 6 initial messages sent
...
1. **Server sends exactly 6 initial messages** immediately after accepting connection (deterministic order):
...
4. **Client sends 4-5 initialization messages** in `onopen` handler:
   - `protocol_version` - validates compatibility
...
sequenceDiagram
    ...
    alt N == 0
        Note over Router: Version compatible, continue
    else N != 0
